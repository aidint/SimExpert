...
namespace SimExpert
{
    public class Discrete : SystemRandom
    {
        ...
        public List<Tuple<long, double>> Probabilitieslong { get; private set; }
        ...
        public Discrete(List<long> values, List<double> cumulative_probabilities)
            : base()
        {
            if (values.Count != cumulative_probabilities.Count)
                throw new Exception("Values and Probabilities sizes are different");
        ...
        {
            double r = Uni_Rand.NextDouble();
            for (int i = 0; i < Probabilitieslong.Count; i++)
            {
                if (r < Probabilitieslong[i].Item2)
                    return Probabilitieslong[i].Item1;
            }
            return Probabilitieslong[Probabilities.Count - 1].Item1;
        }
    }
}
